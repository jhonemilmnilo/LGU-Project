        ...
        <div className="space-y-4 md:space-y-10 pb-20">
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-3 md:gap-4">
                        ...
                        <div className="space-y-0.5">
                            <h1 className="text-2xl md:text-5xl font-black text-slate-900 dark:text-white uppercase italic tracking-tighter leading-none">Explore Gallery</h1>
                            <p className="text-[8px] md:text-[10px] font-bold text-primary uppercase tracking-[0.3em] ml-1">Likas na Yaman</p>
                        </div>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4 md:gap-10 px-4 md:px-0 pt-2 md:pt-0">
                {filteredTourism.map((item, idx) => (
                    ...
                    >
                        <Link href={`/user/tourism/${item.id}`} className="flex flex-col h-full">
                            {/* Image Container */}
                            <div className="relative aspect-[4/3] sm:aspect-[16/10] overflow-hidden">
                                <Image
                                    src={item.imageUrl || "https://images.unsplash.com/photo-1542332213-31f87348057f?auto=format&fit=crop&q=80&w=800"}
                                    alt={item.name}
                                    fill
                            ...
                            <div className="p-3 md:p-8 space-y-2 md:space-y-4 flex-1 flex flex-col justify-between">
                                <div className="space-y-1.5 md:space-y-4">
                                    <div className="space-y-0.5 md:space-y-2">
                                        ...
                                        <div className="flex items-center gap-1 md:gap-2 text-slate-400 group-hover:text-slate-500 transition-colors">
                                            <div className="p-0.5 md:p-1 bg-primary/10 rounded-md">
                                                <Compass className="w-2.5 h-2.5 md:w-4 md:h-4 text-primary shrink-0" />
                                            </div>
                                    ...
                                    <p className="text-[8px] md:text-sm text-slate-500 dark:text-slate-400 font-medium italic line-clamp-2 md:line-clamp-2 leading-relaxed h-6 md:h-10 text-left">
                                        {item.description}
                                    </p>
                                </div>
                                
                                <div className={`flex items-center justify-between mt-auto border-slate-100 dark:border-white/5 ${item.bestTimeToVisit ? "border-t pt-2 md:pt-8" : "md:border-t md:pt-8 hidden md:flex"}`}>
                                    {item.bestTimeToVisit && (
                                        <div className="flex flex-col text-left">
                                            <span className="text-[6px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-0.5 md:mb-1 text-left">Best visit</span>
                                            <span className="text-[9px] md:text-lg font-black text-primary italic tracking-tighter leading-none text-left">{item.bestTimeToVisit}</span>
                                        </div>
                                    )}
            ...
            {filteredTourism.length === 0 && (
                <div className="py-32 flex flex-col items-center justify-center space-y-4 opacity-40 text-center px-6">
                    <Compass className="w-12 h-12" />
                    <p className="font-black uppercase tracking-widest text-xs italic">Adventure loading... Explore soon!</p>
                </div>
            )}
        </div>
    );
