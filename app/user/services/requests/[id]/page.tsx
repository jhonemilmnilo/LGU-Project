    ...
    return (
        ...
        >
            <div className="max-w-5xl mx-auto px-4 md:px-0 pt-4 md:pt-10 space-y-6 md:space-y-12">
                ...
                <div className="sticky top-[70px] md:static z-40 -mx-4 md:mx-0 px-4 md:px-0 pt-2 md:pt-0">
                    <Breadcrumb>
                                        ...
                                        {((isBusinessPermit
                                            ...
                                            doc.url ? (
                                                <a key={i} href={doc.url} target="_blank" rel="noopener noreferrer" className="relative aspect-[4/3] rounded-2xl border border-slate-200 dark:border-white/10 overflow-hidden group/doc hover:shadow-xl transition-all block">
                                                    ...
                                                    <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover/doc:opacity-100 transition-opacity flex items-center justify-center"><div className="w-8 h-8 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center border border-white/30"><ExternalLink className="w-4 h-4 text-white" /></div></div>
                                                    <div className="absolute bottom-2 left-2 right-2"><Badge className="text-[7px] bg-white/95 dark:bg-slate-950/95 text-slate-900 dark:text-white border-none font-black italic tracking-widest uppercase w-full block text-center py-0.5 truncate">{doc.label}</Badge></div>
                                                </a>
                                            ) : (
                                                <div key={i} className="relative aspect-[4/3] rounded-2xl border border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/[0.02] flex flex-col items-center justify-center text-slate-200"><XCircle className="w-6 h-6 mb-1 opacity-40" /><p className="text-[7px] font-black uppercase text-slate-300 italic">Not Required</p></div>
                                            )
                                        ))}
                                    </div>
                                </div>
