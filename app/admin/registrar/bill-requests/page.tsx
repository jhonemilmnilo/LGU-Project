...
export default function BillRequestsList() {
    ...
    const fetchTransactions = useCallback(async () => {
        setLoading(true);
        try {
            // Only fetch transactions that are in FOR_REQUESTING (evaluation/requesting) state
            const res = await getTreasuryTransactions("FOR_REQUESTING");
            if (res.success) {
            ...
            console.error("Failed to load requesting transactions:", err);
    ...
    const formatStatusLabel = (s?: string) => {
        if (!s) return "N/A";
        // Treat both server 'EVALUATED' and treasury 'FOR_REQUESTING' as the Evaluation label
        if (s === "EVALUATED" || s === "FOR_REQUESTING") return "FOR_REQUESTING";
                        ...
                        {loading ? (
                            ...
                            filtered.map((tx, i) => (
                                <TableRow key={tx.id} onClick={() => router.push(`/admin/registrar/${tx.id}`)} className="cursor-pointer hover:bg-slate-50/50">
                                    ...
                                    <TableCell><span className="text-[10px] font-black uppercase italic tracking-wider text-blue-600">{formatStatusLabel(tx.status)?.toUpperCase()}</span></TableCell>
                                    <TableCell>
