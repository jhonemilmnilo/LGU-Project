...
export default function RegistrarDashboard() {
    ...
    const fetchTransactions = useCallback(async () => {
        setLoading(true);
        try {
            // Always fetch only FOR_REQUESTING (evaluation) status for Registrar
            const res = await getTreasuryTransactions("FOR_REQUESTING");
            if (res.success) {
                // Filter to only show Civil Registry transactions
                const civilRegistryTxs = (res.data || []).filter(
                    (tx: any) => tx.type?.category === "Civil Registry"
                );
                setTransactions(civilRegistryTxs);
            } else {
                console.error("[RegistrarDashboard] getTreasuryTransactions failed:", res.error);
                setTransactions([]);
                toast.error(res.error || "Failed to load transactions. Check your permissions.");
            }
        } catch (err) {
            console.error("[RegistrarDashboard] Unexpected error:", err);
            ...
            <div className="bg-white dark:bg-[#151b2b] rounded-3xl border border-slate-200 dark:border-[#2a3040] shadow-2xl shadow-blue-500/5 overflow-hidden ring-1 ring-slate-200 dark:ring-white/5">
                    ...
                    <div className="flex flex-col border-b border-slate-200 dark:border-[#2a3040] bg-slate-50/50 dark:bg-[#151b2b]">
                        ...
                        <div className="p-4 flex flex-col lg:flex-row items-center justify-between gap-4">
                            ...
                            {/* Right controls: Service dropdown + Status dropdown + Refresh */}
