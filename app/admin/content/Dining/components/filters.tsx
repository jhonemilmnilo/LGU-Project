...
export function DiningFilters() {
    ...
    const cuisineOptions = useMemo(() => {
        const cuisines = diningData
            .map(item => item.cuisineType?.trim())
    ...
    return (
        <div className="p-4 border-b border-slate-200 dark:border-[#2a3040] bg-white dark:bg-[#1e2330]">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                <div className="flex flex-wrap flex-1 items-center gap-3">
                    <div className="relative flex-1 min-w-[260px] max-w-sm">
                        ...
                        <Input
                            ...
                            className="pl-9 h-11 bg-slate-50 dark:bg-[#0f1117] border-slate-200 dark:border-[#2a3040] text-slate-900 dark:text-white placeholder:text-slate-500 w-full transition-colors"
                        />
                    </div>

                    <Select value={selectedCuisine} onValueChange={setSelectedCuisine}>
                        <SelectTrigger className="w-full sm:w-[170px] h-11 bg-slate-50 dark:bg-[#0f1117] border-slate-200 dark:border-[#2a3040]">
                    ...
                    <Select value={selectedStatus} onValueChange={setSelectedStatus}>
                        <SelectTrigger className="w-full sm:w-[150px] h-11 bg-slate-50 dark:bg-[#0f1117] border-slate-200 dark:border-[#2a3040]">
                    ...
                    {activeBarangays.length > 0 && (
                        <Select value={currentBarangay || "All"} onValueChange={handleBarangayChange}>
                            <SelectTrigger className="w-full sm:w-[170px] h-11 bg-slate-50 dark:bg-[#0f1117] border-slate-200 dark:border-[#2a3040] font-bold italic text-[11px] uppercase tracking-wider">
                ...
                <Button
                    ...
                    className="w-full lg:w-auto h-11 text-white shadow-lg px-5 rounded-xl font-bold flex items-center justify-center"
